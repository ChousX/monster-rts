...
use rand::prelude::*;
mod movment;
use crate::map::{MyMapSettings, CHUNK_SIZE, TILE_SIZE};
        ...
        app.add_event::<CameraMoveEvent>()
            ...
            .add_system_set(
                SystemSet::on_update(GameState::MainGame)
                    .with_system(movment::move_camera)
                    .with_system(movment::move_camera_gamepad)
                    .label("camera_postion_update"),
            )
            .add_system_set(
                SystemSet::on_update(GameState::MainGame)
                    .after("camera_postion_update")
                    .with_system(camra_bounding),
            )
            .add_system_set(
                SystemSet::on_enter(GameState::MainGame)
                    .with_system(move_camera_to_random_pos)
                    .label("rand_move_camera"),
            );
    }
}
...
pub fn camra_bounding(
    ...
    let (bottom, top) = {
        ...
        (
            (0. + offset.0, -chunk_size.1 + offset.1),
            (chunk_size.0 - offset.0, 0.),
        )
    };
