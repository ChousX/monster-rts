...
pub fn init(
    ...
    if cfg!(debug_assertions){println!("AssetChecker: Inited")}
...
pub fn check(
    ...
    if cfg!(debug_assertions){println!("AssetChecker: Checking")}
    
    if let LoadState::Loaded = server.get_group_load_state(assets.0.iter().map(|h| h.id)) {
        if cfg!(debug_assertions){println!("AssetChecker: Loadded")}    
